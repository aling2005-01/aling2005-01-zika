...
const savedCover = safeGetItem(APP_PREFIX + 'playerCover');
    ...
    const pickLocalSongBtn = document.getElementById('pick-local-song-btn');
    const localSongInput = document.getElementById('local-song-input');
    const localSongHint = document.getElementById('local-song-hint');
    ...
    let pendingLocalSongFile = null;   // 添加模式下选中的本地音频文件
    let editingLocalUrl = null;        // 编辑模式下原歌曲的本地 dataURL
    ...
    function resetLocalSongPick() {
        pendingLocalSongFile = null;
        if (localSongInput) localSongInput.value = '';
        if (localSongHint) {
            localSongHint.classList.remove('show');
            localSongHint.innerHTML = '';
        }
        if (pickLocalSongBtn) {
            pickLocalSongBtn.classList.remove('has-file');
            pickLocalSongBtn.innerHTML = '<i class="fas fa-folder-open"></i> 选择本地文件（mp3 等）';
        }
    }
    ...
    function openEditModal(index) {
        ...
        resetLocalSongPick();
        if (song.isLocal && song.url && song.url.startsWith('data:')) {
            // 本地导入的歌曲：dataURL 过长不塞进输入框，标记沿用，可选新文件替换
            editingLocalUrl = song.url;
            newSongUrl.value = '';
            newSongUrl.placeholder = '（已导入本地文件，选择新文件可替换）';
            localSongHint.innerHTML = '<i class="fas fa-music"></i> 当前为本地导入文件，重新选择可替换';
            localSongHint.classList.add('show');
        } else {
            editingLocalUrl = null;
    ...
    if (pickLocalSongBtn) {
        pickLocalSongBtn.addEventListener('click', () => {
            if (localSongInput) localSongInput.click();
        });
    }
    if (localSongInput) {
        localSongInput.addEventListener('change', () => {
            const file = localSongInput.files && localSongInput.files[0];
            if (!file) { resetLocalSongPick(); return; }
            pendingLocalSongFile = file;
            const sizeMB = file.size / (1024 * 1024);
            pickLocalSongBtn.classList.add('has-file');
            pickLocalSongBtn.innerHTML = '<i class="fas fa-check-circle"></i> ' + file.name;
            localSongHint.innerHTML = '<i class="fas fa-check-circle"></i> 已选择：' + file.name +
                '（' + sizeMB.toFixed(1) + 'MB），点击"添加播放"完成导入';
            localSongHint.classList.add('show');
            // 自动补歌名（去扩展名）与备注，不覆盖已填写内容
            if (!newSongTitle.value.trim()) {
                newSongTitle.value = file.name.replace(/\.[^.]+$/, '');
            }
            if (!newSongSub.value.trim()) {
                newSongSub.value = '本地文件';
            }
        });
    }
    ...
    <div class="pl-header-actions">
        <button class="pl-icon-btn" id="pl-local-import-btn" title="批量导入本地歌曲"><i class="fas fa-file-audio"></i></button>
    ...
    <input type="file" id="pl-local-input" accept="audio/*,.mp3,.m4a,.wav,.ogg,.flac,.aac,.opus" multiple style="display:none">
`;
        ...
        header.querySelector('#pl-local-import-btn').addEventListener('click', (e) => {
            e.stopPropagation();
            header.querySelector('#pl-local-input')?.click();
        });
        header.querySelector('#pl-local-input').addEventListener('change', async (e) => {
            const files = Array.from(e.target.files).filter(f =>
                (f.type && f.type.startsWith('audio/')) || /\.(mp3|m4a|wav|ogg|flac|aac|opus|weba)$/i.test(f.name)
            );
            e.target.value = '';
            if (!files.length) {
                showNotification('没有识别到音频文件', 'warning');
                return;
            }
            showNotification(`正在批量导入 ${files.length} 首歌曲…`, 'info');
            let ok = 0, fail = 0;
            for (const f of files) {
                if (f.size > 25 * 1024 * 1024) { fail++; continue; }  // 单文件上限 25MB
                try {
                    ...
                    console.warn('[music] 导入失败:', f.name, err);
                    fail++;
                }
            }
            if (ok > 0) savePlaylist();
            showNotification(`导入完成：成功 ${ok} 首${fail ? `，失败 ${fail} 首` : ''}`, fail ? 'warning' : 'success');
        });
    ...
    function applySongData(songData) {
        ...
        confirmAddSongBtn.innerText = editModeIndex >= 0 ? "保存修改" : "添加播放";
    ...
    confirmAddSongBtn.addEventListener('click', () => {
        ...
        if (!url && !pendingLocalSongFile && !(editModeIndex >= 0 && editingLocalUrl)) {
            showNotification('请填写音频链接或选择本地文件', 'error');
            return;
        }

        if (pendingLocalSongFile) {
            const sizeMB = pendingLocalSongFile.size / (1024 * 1024);
            if (sizeMB > 25) {
                showNotification('本地文件超过 25MB，无法保存到本地存储', 'error');
                return;
            }
            confirmAddSongBtn.disabled = true;
            confirmAddSongBtn.innerText = '导入中…';
            const file = pendingLocalSongFile;
            const reader = new FileReader();
            reader.onload = () => {
                applySongData({
                    title,
            ...
            reader.onerror = () => {
                confirmAddSongBtn.disabled = false;
                confirmAddSongBtn.innerText = '添加播放';
                showNotification('本地文件读取失败', 'error');
            };
            reader.readAsDataURL(file);
            return;
        }
...
playlist.style.top = (rect.top + (player.classList.contains('collapsed') ? 65 : 155)) + 'px';
    ...
    if (settings.musicPlayerEnabled) {
        ...
        function initCoreListeners() {
            ...
            // ── 主动发送：点击后梦角从字卡库随机抽取一张字卡发来（原"快速回复"） ──
            const quickReplyBtn = document.getElementById('quick-reply-btn');
            if (quickReplyBtn) {
                quickReplyBtn.addEventListener('click', () => {
                    try {
                        let pool = ((typeof customReplies !== 'undefined') ? customReplies : (window._customReplies || [])).slice();
                        try {
                            const raw = localStorage.getItem('disabledReplyItems');
                            if (raw) {
                                const dis = new Set(JSON.parse(raw));
                                pool = pool.filter(r => !dis.has(r));
                            }
                        } catch (e) {}
                        (window.customReplyGroups || []).forEach(g => {
                            if (g.disabled && Array.isArray(g.items)) {
                                const s = new Set(g.items);
                                pool = pool.filter(r => !s.has(r));
                            }
                        });
                        pool = pool.map(String).map(s => s.trim()).filter(Boolean);
                        if (!pool.length) {
                            if (typeof showNotification === 'function') showNotification('回复库为空，先添加一些字卡吧', 'info');
                            return;
                        }
                        const text = pool[Math.floor(Math.random() * pool.length)];
                        // 梦角发送：显示"正在输入"后用 addMessage 以对方身份发出
                        if (typeof window._partnerSayCard === 'function') {
                            window._partnerSayCard(text);
                        } else if (typeof simulateReply === 'function') {
                            simulateReply();
                        }
                    } catch (e) { console.warn('[quick-reply] failed', e); }
                });
            }
            ...
            if (moreBtn && morePanel) {
                moreBtn.addEventListener('click', (e) => {
                    ...
                    moreBtn.classList.toggle('active', morePanel.style.display === 'block');
                });
            }
            if (morePanelClose && morePanel) {
                morePanelClose.addEventListener('click', (e) => {
                    e.stopPropagation();
            ...
            // 点击其他区域关闭更多面板
            document.addEventListener('click', (e) => {
                if (morePanel && morePanel.style.display === 'block' && !e.target.closest('#more-panel') && !e.target.closest('#more-btn')) {
                    morePanel.style.display = 'none';
                    if (moreBtn) moreBtn.classList.remove('active');
                }
            });

            // 更多面板：图片（复用 attachment 的发送图片弹窗）
            const moreImageBtn = document.getElementById('more-image-btn');
            if (moreImageBtn) {
                moreImageBtn.addEventListener('click', (e) => {
                    e.stopPropagation();
                    if (morePanel) morePanel.style.display = 'none';
                    if (moreBtn) moreBtn.classList.remove('active');
                    if (DOMElements.attachmentBtn) DOMElements.attachmentBtn.click();
                });
            }
            ...
            if (moreDiceBtn) {
                moreDiceBtn.addEventListener('click', (e) => {
                    e.stopPropagation();
                    if (morePanel) morePanel.style.display = 'none';
                    if (moreBtn) moreBtn.classList.remove('active');
                    if (typeof window._openDiceRoll === 'function') window._openDiceRoll();
                });
            }
