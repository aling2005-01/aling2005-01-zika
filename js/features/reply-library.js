...
function _renderListContentOnly() {
    ...
    // 语音字卡 tab：由 voice-cards.js 自治渲染
    if (currentMajorTab === 'reply' && currentSubTab === 'voicecards') {
        if (window.VoiceCards) window.VoiceCards.renderInto(list);
        return;
    }
...
function renderReplyLibrary() {
    ...
    // 语音字卡 tab：专属工具栏 + 列表由 voice-cards.js 渲染，隐藏通用工具栏
    if (currentMajorTab === 'reply' && currentSubTab === 'voicecards') {
        const vcTb = document.getElementById('batch-ops-toolbar');
        if (vcTb) vcTb.style.display = 'none';
        if (window.VoiceCards) window.VoiceCards.renderInto(list);
        return;
    }
...
function initReplyLibraryListeners() {
    ...
    if (addBtn) {
        addBtn.addEventListener('click', () => {
            ...
            if (currentSubTab === 'voicecards') {
                if (window.VoiceCards) window.VoiceCards.openFilePicker();
                return;
            }
