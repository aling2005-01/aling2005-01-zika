...
(function () {
    ...
    ready(function init() {
        ...
        document.body.addEventListener('click', async (e) => {
            ...
            // ── 语音字卡 / 真实音频：优先播放原始 mp3 ──
            // 1) 消息携带 voiceCardId → 从语音字卡库取音频
            // 2) voice.url 为 data:/blob: 音频 → 直接播放
            const _msgVC = msgId ? findMessage(msgId) : null;
            if (_msgVC && _msgVC.voice) {
                let _realUrl = '';
                if (_msgVC.voice.url && (/^data:audio/i.test(_msgVC.voice.url) || /^blob:/i.test(_msgVC.voice.url))) {
                    _realUrl = _msgVC.voice.url;
                } else if (_msgVC.voice.voiceCardId && window.VoiceCards) {
                    const _card = window.VoiceCards.getById(_msgVC.voice.voiceCardId);
                    if (_card && _card.audio) _realUrl = _card.audio;
                }
                if (_realUrl) {
                    currentBubble = bubble;
                    bubble.classList.add('playing');
                    const audio = new Audio(_realUrl);
                    _currentAudio = audio;
                    audio.onended = () => {
                    ...
                    audio.onerror = () => {
                        ...
                        if (currentBubble === bubble) currentBubble = null;
                        if (_currentAudio === audio) _currentAudio = null;
                        if (typeof showNotification === 'function') showNotification('语音字卡播放失败', 'error');
                    };
                    audio.play().catch(() => {
                        bubble.classList.remove('playing');
                        if (currentBubble === bubble) currentBubble = null;
