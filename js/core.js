...
const LIBRARY_CONFIG = {
    reply: {
        ...
        tabs: [
            ...
            { id: 'voicecards', name: '语音字卡', mode: 'list' }
        ]
    },
    atmosphere: {
        title: "氛围感配置",
...
if (partnerPersonas && partnerPersonas.length > 0 && Math.random() < 0.3) {
            ...
            // 语音字卡可作为独立输出源：文字库为空但语音字卡库非空时仍可回复
            const _vcReady = !!(window.VoiceCards && typeof window.VoiceCards.count === 'function' && window.VoiceCards.count() > 0);
            ...
            const replyPoolOnce = customReplies
                ...
                .map(r => String(r || '').trim())
                .filter(Boolean);
            if (!replyPoolOnce.length && !_vcReady) {
            ...
            for (let i = 0; i < replyCount; i++) {
                ...
                setTimeout(() => {
                    ...
                    // ── 语音字卡：按用户设定的概率本条回复以语音形式发出 ──
                    // 概率在「自定义回复 → 语音字卡」中可自行调整（默认 40%）
                    // 消息里只存 voiceCardId 引用，播放时由 listeners-voice.js 从字卡库取真实音频
                    if (window.VoiceCards && typeof window.VoiceCards.count === 'function'
                        && window.VoiceCards.count() > 0 && Math.random() < window.VoiceCards.probability) {
                        const vc = window.VoiceCards.pickRandom();
                        if (vc && vc.audio) {
                            const vcText = (vc.desc || vc.name || '').trim();
                            addMessage({
                                id: Date.now() + i,
                            ...
                            if (typeof window._sendPartnerNotification === 'function') {
                                window._sendPartnerNotification(settings.partnerName || '对方', '🎤 [语音] ' + (vcText || '语音消息'));
                            }
                            playSound('message');
                            return;
                        }
